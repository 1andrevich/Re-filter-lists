...
def read_domains_from_file(file_path="sum/input/ooni_domains.lst"):
    try:
...
def main():
    ...
    domains = read_domains_from_file("sum/input/ooni_domains.lst")
    if not domains:
        return
    ...
    writer_thread = threading.Thread(target=write_cidrs_to_file, args=("sum/input/ooni_ips.lst",))
    writer_thread.start()
